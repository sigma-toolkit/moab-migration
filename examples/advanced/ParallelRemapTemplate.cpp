...
class RemapConfiguration
{
  public:
    RemapConfiguration( std::string source_mesh, std::string target_mesh, std::optional< std::string > output_mesh,
                        ...
                        std::string read_options, std::string weights_id, Discretization source_discretization,
                        Discretization target_discretization, std::string source_field, std::string target_field,
        ...
        : source_mesh( std::move( source_mesh ) ), target_mesh( std::move( target_mesh ) ), output_mesh( std::move( output_mesh ) ),
          ...
          source_discretization( std::move( source_discretization ) ), target_discretization( std::move( target_discretization ) ),
...
{
    RemapConfiguration configuration = parse_command_line( argc, argv );
